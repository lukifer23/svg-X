...
interface FormatInfo {
  ...
  mime: string;
  ext: string;
  generate: (svg: string, document: VectorDocument) => string;
}
...
const FORMAT_INFO: Record<ExportFormat, FormatInfo> = {
  ...
  eps: {
    ...
    ext: "eps",
    generate: (_svg, document) => serializeEps(document),
  },
  dxf: {
    label: "DXF",
    mime: "application/dxf",
    ext: "dxf",
    generate: (_svg, document) => serializeDxf(document),
  },
  json: {
    label: "JSON Paths",
    mime: "application/json",
    ext: "json",
    generate: (_svg, document) => serializePathJson(document),
  },
};
...
const DownloadButton: React.FC<DownloadButtonProps> = ({
  ...
  const browserDownload = (format: ExportFormat) => {
    ...
    const content = info.generate(svg, vectorDocument);
  ...
  const handleDownload = async (format: ExportFormat = "svg") => {
    ...
    if (window.electronAPI?.saveExport) {
      try {
        const content = FORMAT_INFO[format].generate(svg, vectorDocument);
        ...
        showSaveError(
          `Native save failed: ${msg}. Falling back to browser download.`,
        );
  ...
  return (
    <div className={positionClasses}>
      {saveError && (
        <div
          className="mb-2 px-4 py-2 bg-amber-100 border border-amber-300 text-amber-800 text-xs rounded-lg shadow-md max-w-md text-center"
          aria-live="polite"
        >
          {saveError}
        </div>
      ...
      <div className="flex items-stretch" ref={menuRef}>
        ...
        >
          <ChevronDown
            className={`w-4 h-4 transition-transform ${formatMenuOpen ? "rotate-180" : ""}`}
          />
        </button>
        ...
        {formatMenuOpen && (
          <div className="absolute bottom-full mb-2 right-0 bg-white border border-gray-200 rounded-lg shadow-xl overflow-hidden min-w-[160px] z-30">
            {(Object.entries(FORMAT_INFO) as [ExportFormat, FormatInfo][]).map(
              ([fmt, info]) => (
                <button
                  key={fmt}
                ...
                >
                  <span className="font-mono text-xs bg-gray-100 px-1.5 py-0.5 rounded text-gray-500 w-10 text-center">
      ...
      >
        {copied ? (
          <Check
            className={`${isMobile ? "w-4 h-4" : "w-5 h-5"} mr-2 text-green-500`}
          />
        ) : (
