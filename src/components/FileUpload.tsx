...
const FileUpload: React.FC<FileUploadProps> = ({
  ...
  const processFile = (file: File) => {
    ...
    if (
      ...
      setError(
        "Unsupported file type. Please select a PNG, JPG, GIF, BMP, WEBP, AVIF, HEIC, or TIFF image.",
      );
      return;
    }
    ...
    if (sizeMB > MAX_SIZE_MB) {
      setError(
        `File is too large (${sizeMB.toFixed(1)} MB). Maximum size is ${MAX_SIZE_MB} MB.`,
      );
      return;
    }
    ...
    if (sizeMB > 10) {
      setSizeWarning(
        `Large file (${sizeMB.toFixed(1)} MB) — processing may take longer.`,
      );
    }
    ...
    <div className="flex flex-col gap-3">
      ...
      >
        ...
        <div className="flex flex-col items-center justify-center">
          ...
          <h3
            className={`${isMobile ? "text-lg" : "text-xl"} font-medium text-gradient mb-1 sm:mb-2`}
          >
